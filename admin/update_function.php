                ...
                <div class="col-xl">
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                      ...
                      <?php 
                      $f_id=$_GET['f_id'];
$select_function="SELECT * FROM functions WHERE f_id='$f_id'";
...
$row_cselect_function=$cselect_function->fetch(PDO::FETCH_ASSOC);
                       ?>
                      <form method="POST" id="form_update_comment">
                        <div class="mb-3">
                          <label class="form-label">Function Name</label>
                          <input type="text" class="form-control" value="<?php echo $row_cselect_function['function_name']; ?>" name="function_name"  placeholder="Function Name" />
                          <input type="hidden" name="f_id" value="<?php echo $row_cselect_function['f_id']; ?>">
                        </div>                    
                        <button type="submit" class="btn btn-primary" id="admin_update_functions">Update Function</button>
                      </form>
                    </div>
                  </div>
                </div>
        ...
        $(document).ready(function () {
          ...
          $("#form_update_comment").validate({
            ...
            unhighlight: function (element, errorClass, validClass) {
             ...
             var f = $("#form_update_comment")[0];
              var form = new FormData(f);
              $.ajax({
                url: '../php/update_functions.php',
                type: 'POST',
                data: form,
                cache: false,
                processData: false,
                contentType: false,
                beforeSend: function () {
                    $('#admin_update_functions').html('Progress <i class="fa fa-solid fa-spinner fa-spin"></i>');
                    $('#admin_update_functions').attr("disabled", true);
                ...
                // }
                 ...
                 complete: function () {
                    setTimeout(function () {
                        ...
                        $('#admin_update_functions').html('submit');
                        $('#admin_update_functions').attr("disabled", false);
                    }, 200);
                },
