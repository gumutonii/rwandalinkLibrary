  ...
  function study_mode(str){
    if (str == '') {
      document.getElementById("study").innerHTML = "";
       ...
       xmlhttp.onreadystatechange = function() {
      if (this.readyState  == 4 && this.status == 200) {
        document.getElementById("study").innerHTML = this.responseText;
      }
       };
       xmlhttp.open("GET","study_mode.php?o_id="+str,true);
       xmlhttp.send();
    }
  }
</script>
                ...
                <div class="col-12 col-md-8 col-lg-12 order-3 order-md-2">
                  ...
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                      ...
                      <form method="POST" id="student_information">
                        <div class="row">
...
$cselect_academic_year=$connect->prepare($select_academic_year);
$cselect_academic_year->execute();
while($row_cselect_academic_year=$cselect_academic_year->fetch(PDO::FETCH_ASSOC)){
                             ?>
                             <option value="<?php echo $row_cselect_academic_year['ac_id']; ?>"><?php echo $row_cselect_academic_year['ac_name'];?></option>
                           <?php } ?>
                              </select>
                        </div>
                        ...
                        <div class="input-group">
                          <select class="form-control" name="y_id">
                                ...
                                <?php 
$select_year_of_study="SELECT * FROM year_study";
$cselect_year_of_study=$connect->prepare($select_year_of_study);
$cselect_year_of_study->execute();
while($row_cselect_year_of_study=$cselect_year_of_study->fetch(PDO::FETCH_ASSOC)){
                                 ?>
                                <option value="<?php echo $row_cselect_year_of_study['y_id'] ?>">Year <?php echo $row_cselect_year_of_study['y_name'];  ?></option>
                              <?php } ?>
                              </select>
...
while($row_cselect_department=$cselect_department->fetch(PDO::FETCH_ASSOC)){
                        ...
                        <div class="row justify-content-end">
                          <div class="col-sm-12">
                            <button type="submit" class="btn btn-primary" id="view_student">View Student</button>
                          </div>
                        </div>
...
fetch();
          ...
          $("#student_information").validate({
            ...
            unhighlight: function (element, errorClass, validClass) {
             ...
             var f = $("#student_information")[0];
              var form = new FormData(f);
              $.ajax({
                url: 'academic_student.php',
                type: 'POST',
                data: form,
                cache: false,
                processData: false,
                contentType: false,
                beforeSend: function () {
                    $('#view_student').html('Progress  <i class="fa fa-solid fa-spinner fa-spin"></i>');
                    $('#view_student').attr("disabled", true);
                },
              ...
              window.location.href=dataResult;
                 ...
                 complete: function () {
                    setTimeout(function () {
                        ...
                        $('#view_student').html('View');
                        $('#view_student').attr("disabled", false);
                    }, 200);
                },
