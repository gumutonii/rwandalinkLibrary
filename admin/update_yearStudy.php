                ...
                <div class="col-xl">
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                      ...
                      <?php 
$select_year="SELECT * FROM year_study WHERE y_id='".$_GET['y_id']."'";
$cselect_year=$connect->prepare($select_year);
$cselect_year->execute();
$row_cselect_year=$cselect_year->fetch(PDO::FETCH_ASSOC);
                       ?>
                      <form method="post" id="update_YearOfStudy">
                        <div class="mb-3">
                          <label class="form-label">Year Of Study Name</label>
                          <input type="text" class="form-control" name="y_name"  placeholder="Year Of Study Name" value="<?php echo $row_cselect_year['y_name']; ?>">
                        </div>
                        <input type="hidden" name="y_id" value="<?php echo $row_cselect_year['y_id']; ?>">

                        <button type="submit" class="btn btn-primary" id="update_YearOfStudys">Update Year Of Study</button>
                      </form>
                    </div>
                  </div>
                </div>
        ...
        $(document).ready(function () {
          ...
          $("#update_YearOfStudy").validate({
            ...
            unhighlight: function (element, errorClass, validClass) {
             ...
             var f = $("#update_YearOfStudy")[0];
              var form = new FormData(f);
              $.ajax({
                url: '../php/update_year_study.php',
                type: 'POST',
                data: form,
                cache: false,
                processData: false,
                contentType: false,
                beforeSend: function () {
                    $('#update_YearOfStudys').html('Progress <i class="fa fa-solid fa-spinner fa-spin"></i>');
                    $('#update_YearOfStudys').attr("disabled", true);
                ...
                // }
                 ...
                 complete: function () {
                    setTimeout(function () {
                        ...
                        $('#update_YearOfStudys').html('Update Academic Year');
                        $('#update_YearOfStudys').attr("disabled", false);
                    }, 200);
                },
